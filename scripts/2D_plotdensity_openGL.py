...
class GPULinePlot:
    ...
    def __init__(self, width=1280, height=800, title="GPU Lines — instanced, clip, LOD (fixed)"):
        ...
        self._density_fbo = None
        self._density_tex = None
        self._density_prog = None
        self.density_gain = 1.0   # multiplicador de intensidad

    def _init_density_fbo(self):
        """Initialize framebuffer for density accumulation."""
        ...
        if glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE:
            raise RuntimeError("Density framebuffer incomplete")
    ...
    def _init_gl(self):
        ...
        # 🟩 Initialize framebuffer for density visualization
    ...
    def _on_key(self, win, key, sc, action, mods):
        ...
        #    print("enable_subsample =", self.enable_subsample)
            ...
            self._dirty = True
            print("Density mode:", self.show_density)
    ...
    def save_current_view(self, filename=None):
        ...
        with axis-scaled matplotlib labels.
        """
        import numpy as np
        ...
        if self.show_density:
            if filename is None:
                filename = "density_view.png"
            glBindFramebuffer(GL_FRAMEBUFFER, self._density_fbo)
            # We stored density as single-channel float
            ...
            img = np.log10(1 + img)
            ogcmap = mpl.colormaps['YlOrRd']
            cmap = LinearSegmentedColormap.from_list('', ['white', *[ogcmap(i) for i in np.linspace(1/21,1,20)]])
            ...
            cmap = None
            print("Saving standard RGB framebuffer...")
        ...
        ax.set_ylim([0,0.8])
        ax.vlines(get_bulk_classic(-14.253282664300396, -6.81835453297334/2),0.0,400,colors='b',lw=1)
        ax.fill_between([-2.,get_bulk_classic(-14.253282664300396, -6.81835453297334/2)],[400,400],fc='b',alpha=0.15, zorder=1)
        ax.text(-0.8,0.7,'Cu$_2$O-bulk',va='center',ha='center', bbox=dict(facecolor=(1,1,1), edgecolor=(0,0,0)))
        ...
        if img.ndim == 2:  # density map
            ...
            cbar.set_label("log₁₀(1 + Density of State per pixel)")
        else:               # standard RGB
            ax.imshow(img, extent=(l, r, b, t), origin="lower")
        

        funct = objective_min_distance_to_electrochemicalhull(
        ...
        # --- 2. Load dataset ---
        p = Partition()
#        p.read_files("./out_ideal.xyz")
#        print(p.size)
        p.read_files("./Lit.xyz")
        data,_ = funct(p)

        for i, val in enumerate(data):
            L = p.containers[i].AtomPositionManager.latticeVectors
            val /= float(np.linalg.norm(np.cross(L[:, 0], L[:, 1])))
            print(val)
            val-=0.08119767
            val*=2
            ax.plot([-1,0.5],val,'k',lw=1)
#       
#        data = np.loadtxt('/home/hero/data/low_E_front/end_32_32_1.txt')
#        ax.plot(data[0],(data[1] - 0.08119767)*2, 'k', zorder=3)

        plt.savefig(filename, dpi=200, bbox_inches="tight")
        plt.close()
        glBindFramebuffer(GL_FRAMEBUFFER, 0)

        print(f"> Saved axis-calibrated view as {filename}")
...
if __name__ == "__main__":
    ...
    def plot(path, add=True, colors=[1, 0, 0]):
        ...
        # Alpha: more opaque for hull structuresq
        cols[:, 3] = np.where(on_hull, 0.2, 0.2)

        # Optional: brighten RGB for hull members
        cols[on_hull, :3] = np.clip(np.array(colors) * 1.3, 0, 1)
        cols[:,  3] = 0.02 # np.e**( -(min_distances1-np.min(min_distances1)) / (0.0258/298 * 300) ) #  <<<<
        #cols[:,  3] /= np.linalg.norm(cols[:,  3])
        #cols[:,  3] /= np.max(cols[:,  3])

        # compute normalized composition (Cu, O, H)
        species, order = p.get_all_compositions(return_species=True)
        idx = {el: i for i, el in enumerate(order)}
        Cu = species[:, idx.get('Cu', 0)] 
        Cu -= np.min(Cu, axis=0)  
        O  = species[:, idx.get('O', 0)]
        H  = species[:, idx.get('H', 0)]
        sum_all = Cu + O + H + 1e-9

        cols[:,:3] = np.column_stack([O/sum_all, Cu/sum_all, H/sum_all]).astype(np.float32)
    ...
    # 1st PLOT add=False
    #path = '/home/hero/data/cleaned_data_base/end_08_08_1'
    #plot(path,add=False,colors=[1,0,0])

    #path = '/home/hero/data/data_base/end_32_32_1_inclusive_GA_11'
    path = '/home/hero/data/re_data_base/32x32'
    plot(path,add=True,colors=[1,0,0])

    #path = '/home/hero/data/general_scripts/test_db/'
    #plot(path,add=False,colors=[1,0,0])
